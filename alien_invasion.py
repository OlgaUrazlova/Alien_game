...
class AlienInvasion:
    ...
    def _create_fleet(self):
        """Создание флота вторжения и вычисление количества пришельцев в ряду.
        Ширина между равна ширине 1 пришельца"""
        alien = Alien(self)
        alien_width, alien_height = alien.rect.size
        available_space_x = self.settings.screen_width - (alien_width * 2)
        number_aliens_x = available_space_x // (alien_width * 2)

        # Определяем количество рядов, которые помещяются на экране
    ...
    def _check_bullet_alien_collision(self):
        """Обработка столкновения снарядов и пришельцев"""
    ...
    def _change_fleet_direction(self):
        """Флот опускается и меняет направления"""
        for alien in self.aliens.sprites():
            alien.rect.y += self.settings.fleet_drop_speed
        self.settings.fleet_direction *= -1

    def _ship_hit(self):
        """Обработка столкновение пришельца и корабля"""
    ...
    def _check_aliens_bottom(self):
        """Проверяем положение пришельца у нижнего края"""
    ...
    def _update_aliens(self):
        ...
        # Проверка столкновения пришельца и корабля
